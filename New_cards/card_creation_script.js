...
frontCtx.font = "bold 100px GilmerMedium";
  ...
  function saveCard() {
   ...
   async function saveComboCards(cardData, frontImageData, backImageData) {
    ...
    //   });
     ...
     try {
    // Save dog card if blueDog toggle is true
    if (comboToggleStates.blueDog) {
      const dogResponse = await fetch("save_card.php", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (dogResult.success) {
        console.log("Dog card saved:", dogResult);
      } else {
        console.warn("Failed to save dog card:", dogResult.message);
      }
    }

    // Save service dog handler card if handler toggle is true
    if (comboToggleStates.handler) {
      const handlerResponse = await fetch("save_card.php", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (handlerResult.success) {
        console.log("Service dog handler card saved:", handlerResult);
      } else {
        console.warn(
          "Failed to save service dog handler card:",
          handlerResult.message
        );
      }
    }

    showSuccessMessage("Selected cards saved successfully!");
    downloadComboCardsLocally();
  } catch (error) {
    console.error("Error saving combo cards:", error);
    alert("Error saving combo cards: " + error.message);
...
*/
  ...
  function resetCard() {
    ...
    document.getElementById("regularCardDisplay").style.display = "flex !important";
...
function toggleCardDisplay(isCombo = false) {
  ...
  if (isCombo) {
    ...
    regularDisplay.style.display = "flex"; // or "flex" depending on your layout
